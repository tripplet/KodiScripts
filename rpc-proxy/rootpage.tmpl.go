...
function getPercentage(){connection.send(JSON.stringify({"id":1,"jsonrpc":"2.0","method":"Player.GetProperties","params":{"playerid":active_playerid,"properties":["percentage","time","totaltime"]}}));clearTimeout(refresh_timer);if(active_playerid!=-1)
...
function hideProgress(){active_playerid=-1;document.getElementById("slider-container-main").style["display"]="none";getActivePlayers();}
...
if("result"in rx){var result=rx["result"];if(result.length==1&&result[0]===Object(result[0])&&"playerid"in result[0])
...
else if(result===Object(result)&&"method"in rx&&rx["method"]=="Player.OnStop")
{hideProgress();}}
