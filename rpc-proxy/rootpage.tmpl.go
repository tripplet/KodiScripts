...
// GENERATED: 2020-11-22 12:54:46.715052 +0100 CET m=+0.003163020
var content = `<!doctype html><meta charset=utf-8><meta name=viewport content="width=device-width,initial-scale=1"><title>Kodi</title><style>*{box-sizing:border-box}:root{--background:rgb(210, 210, 210);--slider-text-color:rgb(0, 71, 95)}@media screen and (prefers-color-scheme:dark){:root{--color:rgb(219, 219, 219);--background:#25282c;--background-popup:var(--background);--slider-text-color:rgb(102, 181, 208)}input{color:var(--color)}input.InputAddOn-field{background-color:var(--background);filter:brightness(1.6)}}html,body{height:100%;background-color:var(--background);margin:0}@media(max-width:750px){.InputAddOn{display:block!important;text-align:center;margin:0 auto}.InputAddOn-field,.InputAddOn-item{padding:1em!important}input[name=url]{width:85%!important}}a{color:#008cba}.container{height:100%;font-family:helvetica neue,Helvetica,Arial,sans-serif;display:-webkit-flex;display:-ms-flexbox;display:flex;flex-direction:column;align-items:center;-webkit-align-items:center;justify-content:center;-webkit-justify-content:center}.InputAddOn{display:-webkit-box;display:-webkit-flex;display:-ms-flexbox;display:flex;margin-bottom:1.5em}#sec-link{text-align:center;margin:0 auto}.InputAddOn-field{-webkit-box-flex:1;-webkit-flex:1;-ms-flex:1;flex:1}.InputAddOn-field:not(:first-child){border-left:0}.InputAddOn-field:not(:last-child){border-right:0}.InputAddOn-item{background-color:#008cba;color:#fff;box-shadow:rgba(0,0,0,.2)0 -2px inset;-webkit-box-shadow:rgba(0,0,0,.2)0 -2px 0 0 inset;font-weight:400;cursor:pointer;font-weight:700;margin-left:-1px}.InputAddOn-item:disabled{background-color:#969696;cursor:default}.InputAddOn-field,.InputAddOn-item{border:1px solid rgba(147,128,108,.25);padding:1em 2em;font-size:12pt;-webkit-appearance:none}.InputAddOn-field:first-child,.InputAddOn-item:first-child{border-radius:2px 0 0 2px}.InputAddOn-field:last-child,.InputAddOn-item:last-child{border-radius:0 2px 2px 0}.sk-wave{margin:10px auto 30px;width:50px;height:40px;text-align:center;font-size:10px;display:none}.sk-wave .sk-rect{background-color:#008cba;height:100%;width:6px;display:inline-block;margin:.5px;-webkit-animation:sk-waveStretchDelay 1.2s infinite ease-in-out;animation:sk-waveStretchDelay 1.2s infinite ease-in-out}.sk-wave .sk-rect1{-webkit-animation-delay:-1.2s;animation-delay:-1.2s}.sk-wave .sk-rect2{-webkit-animation-delay:-1.1s;animation-delay:-1.1s}.sk-wave .sk-rect3{-webkit-animation-delay:-1s;animation-delay:-1s}.sk-wave .sk-rect4{-webkit-animation-delay:-.9s;animation-delay:-.9s}.sk-wave .sk-rect5{-webkit-animation-delay:-.8s;animation-delay:-.8s}@-webkit-keyframes sk-waveStretchDelay{0%,40%,100%{-webkit-transform:scaleY(.4);transform:scaleY(.4)}20%{-webkit-transform:scaleY(1);transform:scaleY(1)}}@keyframes sk-waveStretchDelay{0%,40%,100%{-webkit-transform:scaleY(.4);transform:scaleY(.4)}20%{-webkit-transform:scaleY(1);transform:scaleY(1)}}.slidecontainer{width:100%;display:none}.slider{-webkit-appearance:none;width:calc(100% - 20px);margin-left:10px;height:10px;border-radius:3px;background:#fff;outline:none;opacity:.7}.slider-text{text-align:center;margin:10px;color:var(--slider-text-color)}.slider:hover{opacity:1}.slider::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;width:20px;height:20px;border-radius:50%;background:#008cba;cursor:pointer}.slider::-moz-range-thumb{width:20px;height:20px;border-radius:50%;background:#008cba;cursor:pointer}</style><script>var active_playerid=-1,refresh_timer=null,ws=new WebSocket('ws://smithers.local:9090/jsonrpc');function resetForm(a){var b=document.getElementById("link"),c=document.getElementById("progress"),d=document.getElementById("send-link");a?progress_state="block":progress_state="none",c.style.display=progress_state,d.disabled=a,b.disabled=a}function sendUrl(){resetForm(!0),fetch("/jsonrpc",{headers:{"Content-Type":"application/json"},method:"post",body:JSON.stringify({url:link.value})}).then(function(a){resetForm(!1),link.value=""}).catch(function(a){resetForm(!1),alert(a)})}function getPercentage(){ws.send(JSON.stringify({id:1,jsonrpc:"2.0",method:"Player.GetProperties",params:{playerid:active_playerid,properties:["percentage","time","totaltime"]}})),clearTimeout(refresh_timer),active_playerid!=-1&&(refresh_timer=window.setTimeout(getPercentage,1e3))}function getActivePlayers(){ws.send(JSON.stringify({id:1,jsonrpc:"2.0",method:"Player.GetActivePlayers"}))}function hideProgress(){active_playerid=-1,document.getElementById("slider-container-main").style.display="none"}function getTitle(){ws.send(JSON.stringify({id:1,jsonrpc:"2.0",method:"Player.GetItem",params:{playerid:active_playerid}}))}function seek(a){ws.send(JSON.stringify({id:1,jsonrpc:"2.0",method:"Player.Seek",params:{playerid:active_playerid,value:parseFloat(a)}}))}ws.onopen=function(){getActivePlayers()},ws.onerror=function(a){console.log('Error: '+a.data)},ws.onmessage=function(c){var b=JSON.parse(c.data),a;if(b===void 0)return;if("result"in b){if(a=b.result,a.length==1&&a[0]===Object(a[0])&&"playerid"in a[0])active_playerid=a[0].playerid,getPercentage(),getTitle();else if(a===Object(a)&&"item"in a)document.getElementById("info-text").innerHTML=a.item.label;else if(a===Object(a)&&"percentage"in a){if(a.percentage==0){hideProgress();return}document.getElementById("progress-slider").value=a.percentage,document.getElementById("progress-text").innerHTML=a.time.hours+"h"+a.time.minutes+"m"+a.time.seconds+"s",document.getElementById("slider-container-main").style.display="block"}}else b===Object(b)&&"method"in b&&(b.method=="Player.OnStop"&&hideProgress(),b.method=="Player.OnPlay"&&(active_playerid=b.params.data.player.playerid,getTitle(),getPercentage()),b.method=="Player.OnPause"&&(active_playerid=-1))}</script><div class=container><form><div class=InputAddOn><input name=url id=link class=InputAddOn-field placeholder=http://... autocomplete=off type=url size=50 autofocus required>
